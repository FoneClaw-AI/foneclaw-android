...
| [`daily-device-brief`](daily-device-brief/SKILL.md) | Personal assistant | Daily device, schedule, memo, and system information brief | Example |

**Don't see what you need?** That's where you come in — see [Contributing a Skill](#-contributing-a-skill) below.

Try the sample import flow:
...
The HTTPS install page can open FoneClaw through Android App Links or a
browser fallback. FoneClaw opens Home, previews the execution flow, and waits
for explicit save and enable confirmation.
...
2. **Edit the front matter** — set `name`, `description`, and `version`:
